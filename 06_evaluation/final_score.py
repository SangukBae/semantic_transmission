...
ssim_results = []
lpips_results = []
dists_results = []  # 添加 DISTS 结果列表

clip_generateds = []
psnr_generateds = []
ssim_generateds = []
lpips_generateds = []
dists_generateds = []  # 添加 DISTS 生成结果列表
...
def calculate_clip_similarity(frame_path_1, frame_path_2):
    ...
    if image1.size != image2.size:
        ...
        # print("clip",image1.size, image2.size)
...
def calculate_lpips(frame_path_1, frame_path_2):
    ...
    if image1.shape != image2.shape:
        ...
        # print("lpips",image1.shape, image2.shape)
...
def calculate_dists(frame_path_1, frame_path_2):
    ...
    if image1.size != image2.size:
        image2 = image2.resize(image1.size)  # 修改为使用 image1 的尺寸
        # print("dists",image1.size, image2.size)
...
def calculate_average_scores(video_folder_1, video_folder_2):
    clip_scores, psnr_scores, ssim_scores, lpips_scores, dists_scores = [], [], [], [], []
    ...
    for frame_file in os.listdir(video_folder_1):
        ...
        if os.path.exists(frame_path_1) and os.path.exists(frame_path_2):
            ...
            dists_score = calculate_dists(frame_path_1, frame_path_2)  # 添加 DISTS 计算

            clip_scores.append(clip_score)
            psnr_scores.append(psnr_score)
            ssim_scores.append(ssim_score)
            lpips_scores.append(lpips_score)
            dists_scores.append(dists_score)  # 添加 DISTS 分数

    if clip_scores and psnr_scores and ssim_scores and lpips_scores and dists_scores:
        return {
            'clip': np.mean(clip_scores),
            'psnr': np.mean(psnr_scores),
            'ssim': np.mean(ssim_scores),
            'lpips': np.mean(lpips_scores),
            'dists': np.mean(dists_scores)  # 添加 DISTS 平均值
        }
    else:
        return None
...
def clean_frame_dirs(generated_video_folder, video_name):
    ...
    res_frames_dir = os.path.join(generated_video_folder, 'res_frames', video_name)
    gen_frames_dir = os.path.join(generated_video_folder, 'gen_frames', video_name)
    
    # 使用 os.system 删除文件夹
    os.system(f'rm -rf "{ref_frames_dir}"')
    os.system(f'rm -rf "{res_frames_dir}"')
    os.system(f'rm -rf "{gen_frames_dir}"')
    print(f"Cleaned frame directories for video: {video_name}")


# 主函数
def main(reference_video_folder, result_video_folder, generated_video_folder, output_csv, interval=1):
    results = []

    # 清除之前的帧文件夹
    os.system('rm -rf ' + os.path.join(generated_video_folder, 'ref_frames'))
    os.system('rm -rf ' + os.path.join(generated_video_folder, 'res_frames'))
    ...
    for video_file in os.listdir(reference_video_folder):
        if video_file.endswith('.mp4'):
            ...
            result_video_path = os.path.join(result_video_folder, video_name + "_0000.mp4")
            ...
            extract_frames(result_video_path, res_frames_dir, interval)
            extract_frames(generated_video_path, gen_frames_dir, interval)

            # 计算 result 视频的分数
            print(f"Calculating scores for result video: {video_name}...")
            result_scores = calculate_average_scores(ref_frames_dir, res_frames_dir)
            ...
            if result_scores and generated_scores:
                results.append({
                    'name': video_name,
                    'clip_result': result_scores['clip'],
                    'psnr_result': result_scores['psnr'],
                    'ssim_result': result_scores['ssim'],
                    'lpips_result': result_scores['lpips'],
                    'dists_result': result_scores['dists'],  # 添加 DISTS 结果

                    'clip_generated': generated_scores['clip'],
                    'psnr_generated': generated_scores['psnr'],
                    'ssim_generated': generated_scores['ssim'],
                    'lpips_generated': generated_scores['lpips'],
                    'dists_generated': generated_scores['dists']  # 添加 DISTS 生成结果
                })
                print(f"Scores for {video_name} - Result Video: CLIP={result_scores['clip']}, PSNR={result_scores['psnr']}, SSIM={result_scores['ssim']}, LPIPS={result_scores['lpips']}, DISTS={result_scores['dists']}")
                print(f"Scores for {video_name} - Generated Video: CLIP={generated_scores['clip']}, PSNR={generated_scores['psnr']}, SSIM={generated_scores['ssim']}, LPIPS={generated_scores['lpips']}, DISTS={generated_scores['dists']}")

                clean_frame_dirs(generated_video_folder, video_name)  # 删除帧文件夹

                # 添加到列表中以计算平均值
                clip_results.append(result_scores['clip'])
                psnr_results.append(result_scores['psnr'])
                ssim_results.append(result_scores['ssim'])
                lpips_results.append(result_scores['lpips'])
                dists_results.append(result_scores['dists'])  # 添加 DISTS 结果
    ...
    # 计算平均值
    clip_results_mean = np.mean(clip_results)
    psnr_results_mean = np.mean(psnr_results)
    ssim_results_mean = np.mean(ssim_results)
    lpips_results_mean = np.mean(lpips_results)
    dists_results_mean = np.mean(dists_results)  # 添加 DISTS 平均值
    ...
    print(f"Average Scores for Result Videos: CLIP={clip_results_mean}, PSNR={psnr_results_mean}, SSIM={ssim_results_mean}, LPIPS={lpips_results_mean}, DISTS={dists_results_mean}")
    print(f"Average Scores for Generated Videos: CLIP={clip_generateds_mean}, PSNR={psnr_generateds_mean}, SSIM={ssim_generateds_mean}, LPIPS={lpips_generateds_mean}, DISTS={dists_generateds_mean}")

    # 将平均值添加到结果中
    results.append({
        'name': 'Average',
        'clip_result': clip_results_mean,
        'psnr_result': psnr_results_mean,
        'ssim_result': ssim_results_mean,
        'lpips_result': lpips_results_mean,
        'dists_result': dists_results_mean,  # 添加 DISTS 平均结果

        'clip_generated': clip_generateds_mean,
        'psnr_generated': psnr_generateds_mean,
        'ssim_generated': ssim_generateds_mean,
        'lpips_generated': lpips_generateds_mean,
        'dists_generated': dists_generateds_mean  # 添加 DISTS 生成平均结果
    })

    # 保存结果到 CSV
    with open(output_csv, 'w', newline='') as csvfile:
        fieldnames = [
            'name', 'clip_result', 'psnr_result', 'ssim_result', 'lpips_result', 'dists_result',
            'clip_generated', 'psnr_generated', 'ssim_generated', 'lpips_generated', 'dists_generated'
        ]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--result_video_folder', type=str,  default=os.path.join(os.environ.get("DATA_ROOT",""),"16x24","save_dir_diff_0.35"))
    parser.add_argument('--generated_video_folder', type=str,  default=os.path.join(os.environ.get("DATA_ROOT",""),"save_dir_diff_0.35"))
    parser.add_argument('--output_csv', type=str, default='comparison_scores.csv')
    args = parser.parse_args()

    reference_video_folder = args.reference_video_folder
    result_video_folder = args.result_video_folder
    generated_video_folder = args.generated_video_folder
    output_csv = generated_video_folder + '/' + args.output_csv

    print(reference_video_folder, result_video_folder, generated_video_folder, output_csv)
